    ...
    <div className="space-y-5 max-w-[1400px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Ringkasan bisnis Anda secara real-time.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={refresh}
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors hover:bg-slate-50 disabled:opacity-50"
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {KPI_CARDS.map((card) => {
          ...
          return (
            <div key={card.title} className="rounded-xl p-5 border flex flex-col gap-4"
              style={{ backgroundColor: '#FFFFFF', borderColor: '#E2E8F0' }}
            >
              <div className="flex items-start justify-between">
                <div className="h-10 w-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: card.bg }}>
              ...
              <div>
                {loading
                  ? <><Skeleton w="w-24" h="h-7" rounded="rounded-lg" /><Skeleton w="w-32" h="h-3" rounded="rounded" /></>
                  : <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl border p-5" style={{ backgroundColor: '#FFFFFF', borderColor: '#E2E8F0' }}>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t" style={{ borderColor: '#F1F5F9' }}>
            <div className="text-center">
              {loading ? <Skeleton w="w-20" h="h-5" rounded="rounded" /> : <p className="text-lg font-bold text-slate-800">{formatRp(summary.monthRevenue)}</p>}
              <p className="text-xs text-slate-400">Bulan ini</p>
            </div>
            <div className="text-center">
              {loading ? <Skeleton w="w-16" h="h-5" rounded="rounded" /> : <p className="text-lg font-bold text-slate-800">{formatRp(summary.yearRevenue)}</p>}
              <p className="text-xs text-slate-400">Tahun ini</p>
            </div>
            <div className="text-center">
              {loading ? <Skeleton w="w-16" h="h-5" rounded="rounded" /> : <p className="text-lg font-bold text-slate-800">{formatRp(summary.monthExpense)}</p>}
        ...
        <div className="rounded-xl border p-5" style={{ backgroundColor: '#FFFFFF', borderColor: '#E2E8F0' }}>
          ...
          <div className="space-y-4">
            <div className="rounded-xl p-4" style={{ background: 'linear-gradient(135deg, #3B82F6, #6366F1)' }}>
              <p className="text-xs text-blue-100 mb-1">Total Saldo</p>
              {loading
                ? <Skeleton w="w-32" h="h-7" rounded="rounded-lg" />
                : <p className="text-2xl font-bold text-white">{formatRp(summary.cashBalance)}</p>
              }
            </div>
            {[
          ...
          <div>
            {loading
              ? Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="flex items-center gap-4 px-5 py-3" style={{ borderBottom: i < 3 ? '1px solid #F8FAFC' : 'none' }}>
                ...
                <div key={product.productName} className="flex items-center gap-3 mb-3 last:mb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[10px] text-slate-400 w-20 text-right flex-shrink-0">{product.totalQty} terjual</span>
                    </div>
                  </div>
                  <span className="text-xs font-semibold text-slate-600 flex-shrink-0 w-20 text-right">{formatRp(product.totalRevenue)}</span>
                </div>
              ));
            })()
        }
