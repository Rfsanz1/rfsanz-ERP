  ...
  return (
    <SalesLayout title="Sales Order" subtitle="Kelola pesanan pelanggan">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: C.textDark, margin: '0 0 4px' }}>Sales Order</h2>
          <p style={{ fontSize: 13, color: C.textLight, margin: 0 }}>{rows.length} order terdaftar</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => router.push('/sales/smart-order')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, border: 'none', background: C.primary, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: `1.5px solid ${C.border}` }}>
                {['No. Order', 'Pelanggan', 'Tanggal', 'Items', 'Total', 'Status', 'Aksi'].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: C.textLight }}>Tidak ada order ditemukan</td></tr>
              ) : filtered.map(r => (
                <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}`, transition: 'background .15s' }}
                  onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#F5F3FF')}
                  onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}>
                  <td style={{ padding: '13px 16px', fontWeight: 700, color: C.primary }}>{r.soNumber ?? r.orderNumber}</td>
                  ...
                  <td style={{ padding: '13px 16px', color: C.textMid }}>{r.items ?? r.itemCount ?? '–'} item</td>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <button onClick={() => router.push(`/sales/orders/${r.id}`)}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: `1px solid ${C.border}`, background: '#fff', color: C.primary, fontSize: 12, cursor: 'pointer' }}>
                      <Eye size={12} /> Detail
                    </button>
                  </td>
                </tr>
