...
export default function DriverHomePage() {
  ...
  const today = deliveries.filter(d => ['assigned', 'on_the_way', 'arrived'].includes(d.status));
  const counts = {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: CBG, paddingBottom: 80, opacity: mounted ? 1 : 0, transition: 'opacity .4s', fontFamily: 'Inter,sans-serif', maxWidth: 430, margin: '0 auto' }}>
      <div style={{ background: `linear-gradient(135deg,#334155,${C})`, padding: '20px 20px 72px', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,.7)', margin: '0 0 14px' }}>Ada <strong style={{ color: '#fff' }}>{today.length}</strong> pengiriman menunggu hari ini</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
          {[
            ...
            <div key={s.label} style={{ backgroundColor: 'rgba(255,255,255,.15)', borderRadius: 12, padding: '10px 12px', textAlign: 'center' }}>
      ...
      <div style={{ padding: '0 16px', marginTop: -48, position: 'relative' }}>
        <div style={{ backgroundColor: '#fff', borderRadius: 20, boxShadow: '0 8px 32px rgba(71,85,105,.12)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#1E293B', margin: 0 }}>Tugas Hari Ini</h2>
          ...
          {deliveries.slice(0, 4).map((d, i) => {
            ...
            return (
              <button key={d.id} onClick={() => router.push(`/driver/delivery/${d.id}`)}
                style={{ width: '100%', padding: '14px 20px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'flex-start', gap: 12, borderBottom: i < 3 ? '1px solid #F1F5F9' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11.5, color: '#64748B', margin: '0 0 5px', display: 'flex', alignItems: 'center', gap: 3 }}>
          ...
          {deliveries.length === 0 && (
            <div style={{ padding: 32, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: '#94A3B8', fontWeight: 600, margin: 0 }}>Semua tugas selesai! 🎉</p>
            </div>
          )}
        </div>
      </div>
