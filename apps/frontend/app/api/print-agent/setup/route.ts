...
server.on('error', function(e) {
  if (e.code === 'EADDRINUSE') {
    console.error('Port ' + PORT + ' sudah dipakai. Set AGENT_PORT=6632 untuk ganti port.');
  } else {
...
server.listen(PORT, '0.0.0.0', function() {
  console.log('Print Agent v1.3 jalan di port', PORT);
  console.log('Printer:', getPrinters().join(', ') || '(belum ada - cek: lpstat -p)');
});
`;
...
# Tulis agent.js (konten langsung dari server ERP - dijamin valid JS)
cat > "$DIR/agent.js" << 'AGENT_JS_END'
${AGENT_JS}
AGENT_JS_END

# Verify syntax
$NODE --check "$DIR/agent.js" && echo "Syntax agent.js: OK" || { echo "ERROR: agent.js tidak valid!"; exit 1; }
...
export async function GET() {
  const script = SETUP_SH.replace('${AGENT_JS}', AGENT_JS);
