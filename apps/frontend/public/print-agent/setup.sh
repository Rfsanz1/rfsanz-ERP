#!/bin/bash
# Gentong Mas ERP - Print Agent Setup (all-in-one)
# Cara pakai: curl -s http://IP-ERP:5000/print-agent/setup.sh | sudo bash

set -e
DIR="/opt/gm-print-agent"
SVC="/etc/systemd/system/gm-print-agent.service"

echo "=== Gentong Mas Print Agent Setup ==="
...
if [ -z "$NODE" ]; then
  ...
  NODE=$(which node 2>/dev/null || which nodejs)
fi
echo "Node.js: $NODE ($($NODE --version))"

# Install CUPS jika belum ada
...
# Tulis agent.js langsung (tidak perlu download)
cat > "$DIR/agent.js" << 'JS'
'use strict';
...
function doPrint(printer, html, title, cb) {
  ...
  fs.writeFile(tmp, content, 'utf8', function(err) {
    ...
    exec('lp -d "' + pr + '" -t "' + safe + '" -o media=A4 "' + tmp + '"', { timeout: 15000 }, function(e, out, err2) {
...
var server = http.createServer(function(req, res) {
  ...
  if (req.method === 'GET' && url === '/status') {
    return sendJson(res, 200, { ok: true, version: '1.2', hostname: os.hostname(), printers: getPrinters(), message: 'Print Agent aktif' });
  }
  ...
  res.end('Gentong Mas Print Agent v1.2\nGET /status  POST /print\n');
});
...
server.listen(PORT, '0.0.0.0', function() {
  console.log('Print Agent v1.2 jalan di port', PORT);
  console.log('Printer:', getPrinters().join(', ') || '(belum ada)');
});
JS

echo "agent.js ditulis ke $DIR"

# Verify syntax
$NODE --check "$DIR/agent.js" && echo "Syntax OK" || { echo "ERROR: syntax error di agent.js"; exit 1; }
