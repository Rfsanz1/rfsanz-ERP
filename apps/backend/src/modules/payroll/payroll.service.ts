...
export class PayrollService {
  ...
  async createPeriod(dto: { bulan: number; tahun: number; note?: string }) {
    return this.prisma.payrollPeriod.create({ data: dto as any });
  }
  ...
  async processPayment(periodId: string) {
    ...
    try {
      await this.prisma.journal.create({
        data: {
          nomor, tanggal: new Date(), status: 'POSTED' as const,
          deskripsi: `Beban Gaji ${period.bulan}/${period.tahun}`,
          lines: {
            create: [
              { deskripsi: 'Beban Gaji Gross',      debit: totalGross,   kredit: 0,           accountId: await this.findOrCreateAccountId('5001', 'Beban Gaji', 'EXPENSE') } as any,
              { deskripsi: 'Beban BPJS Employer',   debit: totalBpjsEmpr, kredit: 0,          accountId: await this.findOrCreateAccountId('5002', 'Beban BPJS Employer', 'EXPENSE') } as any,
              { deskripsi: 'Hutang Gaji (net)',      debit: 0,            kredit: totalNet,    accountId: await this.findOrCreateAccountId('2201', 'Hutang Gaji', 'LIABILITY') } as any,
              { deskripsi: 'Hutang BPJS Employee',  debit: 0,            kredit: totalBpjsEmp, accountId: await this.findOrCreateAccountId('2202', 'Hutang BPJS', 'LIABILITY') } as any,
              { deskripsi: 'Hutang BPJS Employer',  debit: 0,            kredit: totalBpjsEmpr, accountId: await this.findOrCreateAccountId('2202', 'Hutang BPJS', 'LIABILITY') } as any,
              { deskripsi: 'Hutang PPh 21',         debit: 0,            kredit: totalPph21,  accountId: await this.findOrCreateAccountId('2203', 'Hutang PPh 21', 'LIABILITY') } as any,
            ],
          },
        } as any,
      });
