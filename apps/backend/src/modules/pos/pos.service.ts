...
export class PosService {
  ...
  async login(username: string, password: string) {
    ...
    const session = await this.prisma.posCashierSession.create({ data: { posUserId: user.id, modalAwal: 0, status: 'open' } });
  ...
  async openSession(dto: any, currentUser: any) {
    ...
    return this.prisma.posCashierSession.create({
      data: { posUserId: user.id, modalAwal: dto.openingCash ?? 0, status: 'open' },
    });
  }
  ...
  async getCustomerLoyalty(customerId: string) {
    ...
    if (!record) {
      record = await this.prisma.customerLoyalty.create({ data: { customerId } });
    }
    return record;
  }
  ...
  async syncStockFromInventory() {
    const posProducts = await this.prisma.posProduct.findMany({ where: { isActive: true } });
    let synced = 0;
  ...
  async getManagementSummary(period: string = 'today') {
    ...
    const activeSessions = await this.prisma.posSession.count({ where: { status: 'open' } }).catch(() => 0);
