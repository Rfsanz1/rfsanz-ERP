...
export class PurchasingService {
  ...
  async addBillPayment(billId: string, dto: any) {
    ...
    const payment = await this.prisma.vendorBillPayment.create({
      data: { vendorBillId: billId, amount: Number(dto.amount), method: dto.method ?? 'transfer', referensi: dto.referensi ?? dto.reference, notes: dto.notes, tanggal: dto.tanggal ? new Date(dto.tanggal) : new Date() } as any,
    });
  ...
  async validatePurchaseReturn(id: string) {
    ...
    for (const item of (ret as any).items ?? []) {
      if (item.productId) {
        ...
        await this.prisma.stockMovement.create({ data: { productId: item.productId, type: 'out', qty: item.qty, note: `Retur pembelian ${(ret as any).noReturn}`, referenceId: (ret as any).id } as any }).catch(() => null);
      }
    }
