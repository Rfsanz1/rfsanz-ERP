...
export class SettingsService {
  ...
  async update(data: Record<string, string>) {
    const ops = Object.entries(data).map(([key, value]) =>
      this.prisma.appSetting.upsert({ where: { key }, update: { value }, create: { key, value } as any })
    );
  ...
  async updateDocumentNumber(docType: string, dto: { prefix?: string; separator?: string; useYear?: boolean; useMonth?: boolean; padLength?: number; lastSeq?: number }) {
    const data = await this.prisma.documentNumberConfig.upsert({
      where: { docType },
      update: dto,
      create: { docType, prefix: dto.prefix ?? '', separator: dto.separator ?? '/', useYear: dto.useYear ?? true, useMonth: dto.useMonth ?? true, padLength: dto.padLength ?? 4, lastSeq: dto.lastSeq ?? 0 } as any,
    });
  ...
  async getCompanyProfile() {
    ...
    if (!profile) {
      profile = await this.prisma.companyProfile.create({
        data: { name: 'Perusahaan Saya' } as any,
      });
    }
  ...
  async updateCompanyProfile(dto: { name?: string; logo?: string; address?: string; phone?: string; email?: string; website?: string; npwp?: string; currency?: string; timezone?: string }) {
    ...
    const data = profile
      ...
      : await this.prisma.companyProfile.create({ data: { name: 'Perusahaan Saya', ...dto } as any });
  ...
  async getWorkflow() {
    ...
    if (!config) {
      config = await this.prisma.workflowConfig.create({ data: {} as any });
    }
  ...
  async updateWorkflow(dto: any) {
    ...
    const data = config
      ...
      : await this.prisma.workflowConfig.create({ data: dto as any });
  ...
  async getDefaultAccounts() {
    ...
    if (!accounts) {
      accounts = await this.prisma.defaultAccounts.create({ data: {} as any });
    }
  ...
  async updateDefaultAccounts(dto: any) {
    ...
    const data = accounts
      ...
      : await this.prisma.defaultAccounts.create({ data: dto as any });
  ...
  async createTax(dto: { kode: string; nama: string; tipe: string; rate: number; accountId?: string }) {
    const data = await this.prisma.tax.create({
      data: { kode: dto.kode, nama: dto.nama, tipe: dto.tipe as any, rate: dto.rate, accountId: dto.accountId } as any,
    });
  ...
  async createCurrency(dto: { code: string; name: string; symbol: string; rate?: number; isDefault?: boolean }) {
    const data = await this.prisma.currency.create({
      data: { code: dto.code, name: dto.name, symbol: dto.symbol, rate: dto.rate ?? 1, isDefault: dto.isDefault ?? false } as any,
    });
  ...
  async getNotifications() {
    ...
    if (!config) {
      config = await this.prisma.notificationConfig.create({ data: {} as any });
    }
    return { data: config, message: 'success' };
  }
  ...
  async updateNotifications(dto: any) {
    ...
    const data = config
      ...
      : await this.prisma.notificationConfig.create({ data: dto as any });
